...
function generatePythonStepWithVariable(
  ...
  if (action === "input") {
    ...
    if (isDateLikeInput) {
      lines.push("    if isinstance(_val, str):");
      lines.push("      _val = _val.strip()");
      lines.push('      if "T" in _val:');
      lines.push('        _val = _val.split("T")[0]');
      lines.push('      if " " in _val:');
      lines.push('        _val = _val.split(" ")[0]');
      lines.push('      if "/" in _val:');
      lines.push('        parts = _val.split("/")');
      lines.push("        if len(parts) == 3:");
      lines.push("          m, d, y = parts");
      lines.push('          _val = y + "-" + m.zfill(2) + "-" + d.zfill(2)');
    }
