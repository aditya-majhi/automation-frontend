...
interface RecordingDetailsTabsProps {
  ...
  projectRecordingUrl?: string | null;
...
const RecordingDetailsTabs: React.FC<RecordingDetailsTabsProps> = ({
  ...
  const pythonBaseScript = useMemo(
    ...
    [stepsWithRuntimePaths, variables, locatorPref, projectRecordingUrl],
  );
      ...
      <div style={styles.tabBody}>
        ...
        {activeTab === "selenium" && (
          <SeleniumScriptView
            ...
            locatorPref={locatorPref}
            language={scriptLang}
            projectRecordingUrl={projectRecordingUrl}
          />
        )}
...
const SeleniumScriptView: React.FC<{
  ...
  projectRecordingUrl?: string | null;
}> = ({ steps, variables, locatorPref, language, projectRecordingUrl }) => {
  let script = "";
  if (language === "python") {
    ...
    script = generateFullJavaWithVariables(
      steps,
      variables,
      locatorPref,
      projectRecordingUrl,
...
//to use the Project Level Recording URL
function resolveScriptStartUrl(
  steps: Step[],
  projectRecordingUrl?: string | null,
): string {
  const projectUrl = String(projectRecordingUrl || "").trim();
  if (projectUrl) {
    return projectUrl.startsWith("http://") || projectUrl.startsWith("https://")
      ? projectUrl
      : "https://" + projectUrl;
  }

  const stepUrl = String(steps[0]?.pageUrl || "").trim();
...
function generateFullPythonScriptWithVariables(
  ...
  const pageUrl = resolveScriptStartUrl(steps, projectRecordingUrl);
...
function generatePythonStepWithVariable(
  ...
  if (action === "click") {
    ...
    const nextInputType = String(
      _nextStep?.inputType ||
        _nextStep?.contextMeta?.inputType ||
